...
export function Jobs({ results }) {
    ...
    return (
        <main className="flex flex-col gap-8 min-w-full">
            {results.length > 0 ? results.slice(page * totalPerPage, (page + 1) * totalPerPage).map(job => {
                return (
                    <a href={`/${job.id}`} className="grid grid-rows-3 grid-cols-[128px,1fr,1fr] gap-x-6 items-center shadow-sm shadow-black bg-white rounded p-3 text-[#334680] cursor-pointer max-md:grid-rows-4 max-md:grid-cols-[128px,minmax(min-content,1fr)]" key={job.id}>
                        <img className="self-start h-32 w-32 row-span-3 max-md:row-span-4" src={logo} alt="logo da compania" />
                        ...
                        <div className='flex gap-6 self-center justify-self-end font-medium text-xs max-md:justify-self-start'>
