...
function Main({guests, location}, ref) {
    ...
    return (
        <main className="flex flex-col gap-8">
          ...
          <span className='text-[#4f4f4f] text-sm font-medium'>{results.length} stays</span>
          </div>
            
            <section className={`${results.length !== 0 ? 'stays grid grid-cols-3 max-md:grid-cols-2 max-sm:grid-cols-1 gap-x-16 gap-y-8 items-center justify-center' : ''}`}>
