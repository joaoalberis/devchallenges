...
export function TodayInfo({location, setLocation}){
    ...
    return (
        <aside className={`${!searchGui && "bg-[url('./assets/background.png')]"} relative flex items-center flex-col gap-6 p-12  bg-[#1E213A] overflow-hidden bg-contain bg-no-repeat w-1/3`}>
            {!searchGui ? <>
            ...
            </div> </> : <SearchPlaces setLocation={setLocation}  setSearchGui={setSearchGui} />}
        </aside>
    )
}